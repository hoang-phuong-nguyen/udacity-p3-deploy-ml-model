...
def test_special_characters(data):
    assert '?' not in data.values, 'There are some special characters in the \
        dataframe'


def test_null(data):
    assert data.shape == data.dropna().shape, 'There are some NULL values in \
        the dataframe'


def test_duplicates(data):
    assert data.shape == data.drop_duplicates(
        keep='first').shape, 'There are some duplicates in the dataframe'
